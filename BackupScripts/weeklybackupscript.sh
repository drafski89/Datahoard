...
LOG_DIR="/home/ubuntu/logs/$DATE/weekly"
...
rsync -av --delete --exclude-from=$EXCLUDE_FILE /mnt/The_Vault/Wedding/ /mnt/backup_drive_1/weekly/Wedding > $LOG_DIR/log_wedding.txt
echo "Wedding done" > $LOG_DIR/done_wedding.txt
echo "Finished Wedding Folder Copy"

echo "Beginning Renee Folder Copy"
rsync -av --delete --exclude-from=$EXCLUDE_FILE /mnt/The_Vault/Renee/ /mnt/backup_drive_1/weekly/Renee > $LOG_DIR/log_renee.txt
echo "Renee Folder done" > $LOG_DIR/done_renee.txt
echo "Finished Renee Folder Copy"

echo "Beginning Andrew Folder Copy"
rsync -av --delete --exclude-from=$EXCLUDE_FILE /mnt/The_Vault/Andrew/ /mnt/backup_drive_1/weekly/Andrew > $LOG_DIR/log_andrew.txt
